...
from gdio.commons import near_yx, near_yx2, objectify, dict_get, timestep_to_datetime
from .definitions.Table_4_4 import UNIT_TIME_RANGE
from .definitions.Table_1_4 import TYPE_PROCESSED_DATA
...
class grib(object):
    ...
    def gb_write(self,
        ...
        data.time = data.ref_time + timestep_to_datetime(data.time, self.__unity(step_type))

        # step calculation
        dt = np.array([0])
        if len(data.time) > 1:
            dt = int((data.time[1] - data.time[0]).total_seconds() / (3600 * self.__unity(step_type)))
