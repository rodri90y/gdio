...
class TestNcFiles(unittest.TestCase):
    ...
    def test_grid_varible_dimension(self):
        ...
        self.assertEqual(self.ds.dataset[0].get('u').value.shape, self.expected_dim,
                         'dimension shape of u variable incorrect')

    def test_grib_levels(self):

        self.assertEqual(self.ds.dataset[0].get('u').level, self.expected_levels,
                         'levels of u variable incorrect')

    def test_grib_level_type(self):

        self.assertEqual(self.ds.dataset[0].get('u').type_level, self.expected_level_type,
    ...
    def test_grid_interpolation(self):
        ...
        a = self.ds.dataset[0].get('u').value[2, 0].flatten()
        b = self.ds.dataset[0].get('u').value[3, 0].flatten()
    ...
    def test_grid_select_coords(self):
        ...
        self.assertEqual(sub_set[0].get('u').value.shape, self.expected_sel,
                         'dimension shape of u variable incorrect')
